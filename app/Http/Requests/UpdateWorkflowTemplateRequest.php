...
class UpdateWorkflowTemplateRequest extends FormRequest
{
    ...
    {
        return $this->user()->can('update', $this->route('alur_rekrutmen'));
    }
    ...
    {
        $template = $this->route('alur_rekrutmen');

        return [
            'name' => ['required', 'string', 'max:255', Rule::unique('workflow_templates', 'name')->ignore($template->id)],
