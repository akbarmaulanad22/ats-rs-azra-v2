<x-layouts.app title="Template Bank Soal - ATS RS Azra">

    <div class="flex items-center justify-between mb-5">
        <div>
            <h1 class="text-xl font-semibold text-gray-900">Template Bank Soal</h1>
            <p class="text-xs text-gray-500 mt-0.5">Kelola koleksi soal yang dapat digunakan di berbagai lowongan</p>
        </div>
        <a
            href="{{ route('template-bank-soal.create') }}"
    ...
    <div class="mb-3">
        <form method="GET" action="{{ route('template-bank-soal.index') }}">
            <div class="flex flex-wrap items-center gap-2">
                ...
                @if (request('q'))
                    <a href="{{ route('template-bank-soal.index') }}" class="py-1.5 text-xs text-gray-400 hover:text-gray-600 transition-colors ease-out duration-150">
                        Reset
                    </a>
                @endif
            </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-primary border-b border-primary/10 text-white">
                        ...
                        <th class="text-left px-3 py-2.5 text-[10px] font-semibold uppercase tracking-wider w-32">Jumlah Soal</th>
                        <th class="text-left px-3 py-2.5 text-[10px] font-semibold uppercase tracking-wider w-32">Dibuat</th>
                            ...
                            <td colspan="4" class="px-4 py-14 text-center">
                                <div class="flex flex-col items-center gap-2.5 max-w-xs mx-auto">
                                    ...
                                    @if (request('q'))
                                        ...
                                        <a href="{{ route('template-bank-soal.index') }}" class="text-xs text-primary hover:text-primary-dark transition-colors ease-out duration-150">
                                            Reset pencarian
                                        </a>
                                    @else
                                        <div>
                                            <p class="text-sm font-medium text-gray-700">Belum ada template</p>
                                            <p class="text-xs text-gray-400 mt-0.5">Buat template bank soal pertama Anda</p>
                                        </div>
                                        <a
                                            href="{{ route('template-bank-soal.create') }}"
                                            class="inline-flex items-center gap-1.5 px-3.5 py-2 bg-primary text-white text-sm font-medium rounded-lg hover:bg-primary-dark transition-colors ease-out duration-150"
