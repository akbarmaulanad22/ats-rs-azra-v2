<x-layouts.app title="MCU - {{ $application->candidate->nama_lengkap }} - ATS RS Azra">
    ...
    <div class="mb-5">
        ...
        <h1 class="text-xl font-semibold text-gray-900">Medical Check-Up (MCU)</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                @if ($mcuStage->status->isAdvanceable())
                    ...
                    <div class="flex items-center gap-2">
                        <input
                            ...
                            class="flex-1 px-2 py-1.5 text-xs border border-gray-200 rounded-lg bg-gray-50 text-gray-600"
                        >
                    </div>

                    <form method="POST" action="{{ route('lowongan.mcu.dokumen', [$lowongan, $application]) }}" enctype="multipart/form-data" class="mt-3">
                        @csrf
        ...
        <div class="lg:col-span-2">
            ...
            <div class="bg-white rounded-xl border border-gray-100 p-5">
                ...
                @else
                    @php
                        $stageBadge = match ($mcuStage->status->value) {
                            'selesai' => ['bg-green-100 text-green-700', 'Selesai'],
                            'gagal' => ['bg-red-100 text-red-600', 'Ditolak'],
                            default => ['bg-gray-100 text-gray-500', $mcuStage->status->value],
                        };
                    @endphp
