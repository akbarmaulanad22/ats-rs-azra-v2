<x-layouts.app title="Skrining CV - {{ $lowongan->judul_posisi }} - ATS RS Azra">
    ...
    <div class="mb-5">
        ...
        <div class="flex items-start justify-between gap-4">
            <div>
                <h1 class="text-xl font-semibold text-gray-900">Skrining CV</h1>
                <p class="text-xs text-gray-500 mt-0.5">
                    {{ $lowongan->judul_posisi }} &mdash; {{ $lowongan->unit->nama }}
                    &mdash;
                    <span class="font-medium">{{ $stageKey === 'skrining_cv_hr' ? 'Skrining CV HR' : 'Skrining CV Kepala Unit' }}</span>
                </p>
            </div>
    ...
    {{-- Filter --}}
    <form method="GET" action="{{ route('lowongan.skrining.index', $lowongan) }}" class="mb-4 flex flex-wrap items-center gap-2">
        @php
            $filterOptions = [
                '' => 'Semua',
                'aktif' => 'Menunggu',
        ...
        @foreach ($filterOptions as $value => $label)
            <a
                href="{{ route('lowongan.skrining.index', array_merge(['lowongan' => $lowongan->id], $value !== '' ? ['status' => $value] : [])) }}"
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach ($applications as $index => $application)
                            @php
                                $stage = $application->screening_stage;
                            ...
                            <tr class="odd:bg-white even:bg-primary/5 hover:bg-primary/10 transition-colors ease-out duration-100">
                                ...
                                <td class="px-3 py-2">
                                    <a
                                        href="{{ route('lowongan.skrining.show', [$lowongan, $application]) }}"
