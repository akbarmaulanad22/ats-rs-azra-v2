<x-layouts.app title="Template Wawancara - ATS RS Azra">

    <div class="flex items-center justify-between mb-5">
        <div>
            <h1 class="text-xl font-semibold text-gray-900">Template Wawancara</h1>
            <p class="text-xs text-gray-500 mt-0.5">Kelola template kriteria penilaian dan kesiapan untuk wawancara</p>
        </div>
        <a
            href="{{ route('template-wawancara.create') }}"
    ...
    <div class="mb-3">
        <form method="GET" action="{{ route('template-wawancara.index') }}">
            <div class="flex flex-wrap items-center gap-2">
                ...
                @if (request('q'))
                    <a href="{{ route('template-wawancara.index') }}" class="py-1.5 text-xs text-gray-400 hover:text-gray-600 transition-colors ease-out duration-150">
                        Reset
                    </a>
                @endif
            </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-primary border-b border-primary/10 text-white">
                        <th class="text-left px-3 py-2.5 text-[10px] font-semibold uppercase tracking-wider">Nama Template</th>
                        <th class="text-left px-3 py-2.5 text-[10px] font-semibold uppercase tracking-wider w-40">Tipe</th>
                        <th class="text-left px-3 py-2.5 text-[10px] font-semibold uppercase tracking-wider w-32">Jumlah Item</th>
                    ...
                    @forelse ($templates as $template)
                        <tr class="odd:bg-white even:bg-primary/5 hover:bg-primary/10 transition-colors ease-out duration-100">
                            <td class="px-3 py-1.5">
                                <span class="font-medium text-gray-800 text-xs">{{ $template->nama }}</span>
                            </td>
                            <td class="px-3 py-1.5">
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium
                                    {{ $template->tipe === \App\Enums\InterviewTemplateType::KriteriaPenilaian ? 'bg-blue-50 text-blue-700' : 'bg-emerald-50 text-emerald-700' }}">
                                    {{ $template->tipe->label() }}
                                </span>
                            </td>
                            <td class="px-3 py-1.5">
                            ...
                            <td colspan="4" class="px-4 py-14 text-center">
                                <div class="flex flex-col items-center gap-2.5 max-w-xs mx-auto">
                                    ...
                                    @if (request('q'))
                                        ...
                                        <a href="{{ route('template-wawancara.index') }}" class="text-xs text-primary hover:text-primary-dark transition-colors ease-out duration-150">
                                            Reset pencarian
                                        </a>
                                    @else
                                        <div>
                                            <p class="text-sm font-medium text-gray-700">Belum ada template</p>
                                            <p class="text-xs text-gray-400 mt-0.5">Buat template wawancara pertama Anda</p>
                                        </div>
                                        <a
                                            href="{{ route('template-wawancara.create') }}"
                                            class="inline-flex items-center gap-1.5 px-3.5 py-2 bg-primary text-white text-sm font-medium rounded-lg hover:bg-primary-dark transition-colors ease-out duration-150"
